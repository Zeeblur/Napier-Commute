...
/*
		...
		if (day==0) {
			...
			if (myFeedback != null) {
				...
				CJourney optionFrom= getCheapestOptionInTime(this.getTransportOptions(requestOut, current));
		...
		//Walk selector
		if (walkTime == -1) {
			ArrayList<CJourney> ops =  this.getTransportOptions(requestIn, TransportMode.WALK);
		...
		if (current != null) {
			...
			ArrayList<CJourney> tmp = this.getTransportOptions(requestIn, current);
			choiceIn = tmp.get(0);
			choiceIn.setCommuter(this);
			choiceIn.setTransportMode(current);
			this.modeIn = current;
			this.modeOut = current;
			tmp = this.getTransportOptions(requestOut, current);
			choiceOut = tmp.get(0);
